...
fn query_flow_metrics(conn: &Connection, filter: &MetricsFilter) -> Result<FlowMetricsDto, String> {
    ...
    let cycle_sum: f64 = cycles.iter().sum();
    // Active time approximation unavailable without status categories per issue; use cycle/lead.
    let lead_sum: f64 = leads.iter().sum();
    let flow_efficiency = if lead_sum > 0.0 {
        Some((cycle_sum / lead_sum).clamp(0.0, 1.0))
    } else {
        None
    };
...
fn query_epic_risk(conn: &Connection, filter: &MetricsFilter) -> Result<Vec<EpicRiskDto>, String> {
    let mut sql = String::from(
        "SELECT r.epic_key, r.risk_score, r.finish_by_probability, r.assumptions_json
    ...
    let rows = stmt
        .query_map(params_from_iter(params.iter().map(|p| p.as_ref())), |row| {
            ...
            let assumptions = parse_assumptions(assumptions_json.as_deref());
            Ok(EpicRiskDto {
                epic_key: row.get(0)?,
                score: row.get::<_, Option<f64>>(1)?.unwrap_or(0.0),
                finish_by_probability: row.get(2)?,
                drivers: assumptions.clone(),
                assumptions,
            })
        })
        .map_err(|e| e.to_string())?
...
fn parse_assumptions(raw: Option<&str>) -> Vec<String> {
